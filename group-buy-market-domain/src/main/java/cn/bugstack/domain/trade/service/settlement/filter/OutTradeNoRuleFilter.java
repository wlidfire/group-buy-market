...
public class OutTradeNoRuleFilter implements ILogicHandler<TradeSettlementRuleCommandEntity, TradeSettlementRuleFilterFactory.DynamicContext, TradeSettlementRuleFilterBackEntity> {
    ...
    public TradeSettlementRuleFilterBackEntity apply(TradeSettlementRuleCommandEntity requestParameter, TradeSettlementRuleFilterFactory.DynamicContext dynamicContext) throws Exception {
        log.info("结算规则过滤-有效的外部交易单号校验 userId{} outTradeNo:{}", requestParameter.getUserId(), requestParameter.getOutTradeNo());
        ...
        if (null == marketPayOrderEntity) {
            log.error("不存在的外部交易单号或用户已退单，不需要做支付订单结算 userId:{}, outTradeNo:{}", requestParameter.getUserId(), requestParameter.getOutTradeNo());
